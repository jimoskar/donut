...
Type objective_function<Type>::operator() ()
{
  ...
  // Indices
  DATA_INTEGER( num_i );   // Number of urban data points in space
  DATA_INTEGER( n_integrationPoints );   // number of integration points
  ...
  DATA_VECTOR( y_i);
  DATA_VECTOR( n_i);
  DATA_MATRIX( w ); // nObsRural x nIntegrationPointsRural weight matrix
  DATA_MATRIX( X_mat);
  ...
  vector<Type> fe_i(num_i*n_integrationPoints); // main effect: alpha
  ...
  vector<Type> projepsilon_i(num_i*n_integrationPoints);
  
  // fixed effects is just alpha in this example
  fe_i = X_mat * beta; // initialize
  ...
  for (int i = 0; i < num_i; i++){
    vector<Type> tmp(n_integrationPoints);
    vector<Type> tmpW(n_integrationPoints);
    for (int j = 0; j < n_integrationPoints; j++){
      thisIndex = i + num_i * j;
      
      // latent field estimate at each obs
      //thisLatentField = fe_iUrban(thisIndex) + projepsilon_iUrban(thisIndex);
      thisLatentField = fe_i(thisIndex) + projepsilon_i(thisIndex);
      ...
      if(!isNA(y_i(i))){
        ...
        if ( flag2 ==0 ){
          // Use Gaussian
          tmpW(j) = thisWeight;
          tmp(j) = dnorm(y_i(i), thisLatentField, nugStd, true);
          //thislik += thisWeight*dnorm(y_iUrban(i), thisLatentField, nugStd, false);
        } else if (flag2 == 2){
          tmpW(j) = thisWeight;
          tmp(j) = dnorm(y_i(i), thisLatentField, nugStd/sqrt(n_i(i)), true);
        } else{
          // Use dbinom_robust function, which takes the logit probability
          tmpW(j) = thisWeight;
          tmp(j) = dbinom_robust( y_i(i), n_i(i), thisLatentField, true);
      ...
      if(!isNA(y_i(i))){
        thislik = robustMix(tmpW, tmp, n_integrationPoints);
        //thislik = tmpUrb[0];
      }else{
        thislik = 0.0;
      }
