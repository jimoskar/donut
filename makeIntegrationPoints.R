...
getIntegrationPoints = function(urban=TRUE, numPoints=ifelse(urban, 16, 16), 
                                scalingFactor, 
                                JInner=5, JOuter=ifelse(urban, 0, 1), 
                                integrationPointType=c("mean", "midpoint"), 
                                verbose=TRUE) {
  integrationPointType = match.arg(integrationPointType)
  
  M = JInner + JOuter
  ms = c(1, rep(floor((numPoints - 1) / (M - 1)), M - 1)) # number of points in each "ring"
  msInner = ms[1:JInner]
...
makeAllIntegrationPoints = function(coords, urbanVals, 
  ...
  if(!is.null(adminMap)) {
    ...
    if(adm.level == 2){
      counties <- adminMap$NAME_2
    } else{
      counties <- adminMap$NAME_1
    }
    dict <- c()
